...
```
検索でヒット 135 本          ← video_queries の語で検索した結果
  ↓ 商品紹介ではないと判定 41 本を除外  (紹介度 0.35 未満)
分析対象の紹介動画 94 本      ← ここから先の集計はすべてこの 94 本だけ
  ↓
うち商品リンク付き 56 本      ← どの商品の紹介動画かが確定
  ↓
導出できた商品 6 件           ← 撮る候補
```
...
検索に使う語は `config.yaml` の `video_queries`、
除外のしきい値は `min_product_intent` で変更できます。
