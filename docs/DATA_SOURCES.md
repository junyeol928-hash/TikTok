...
> **v2 での方針変更**: 主力を Creative Center から
> **tiktok.com の商品紹介動画そのもの** に変更しました。
> 「どの商品で撮るか」を決めるのに必要なのは全体集計ではなく、
> 実際に伸びている紹介動画の成績だからです。
> Creative Center は全体トレンドの補助として残しています。
...
## 5. TikTok Web の公開ページ 【★本ツールの主力】

検索結果ページ・ハッシュタグページから、商品紹介動画そのものを集める方法です。

- **短所**: `msToken` / `X-Bogus` 等の署名が必要で、対策も頻繁に更新される
- **対策**: HTTP で直叩きせず、**実ブラウザにページを開かせて XHR を傍受**する。
  署名は TikTok 自身のフロントエンドが作るので、仕様変更に強い
- 取れるもの: 再生数・いいね・コメント・シェア・**保存数**・投稿日時・
  投稿者・ハッシュタグ・**TikTok Shop の商品リンク**

**保存数が取れるのが大きい。** 商品紹介動画では、保存率が購買意欲に最も近い
シグナルになります (「あとで買う」ために保存される)。
販売数そのものは取れなくても、保存率で代替できます。

商品リンク付きの動画をまとめることで、
「その商品を紹介した動画が何本あり、どれくらい伸びているか」が分かります。
これが `tiktok_video` collector と rollup がやっていることです。
